...
internal class HTTPLiveRecordManager
{
    ...
    private async Task<bool> RecordStreamAsync(StreamSpec streamSpec, IProgress<double> task, SpeedContainer speedContainer)
    {
        //task.MaxValue = 1;
        //task.StartTask();

        var name = streamSpec.ToShortString();
        var dirName = $"{DownloaderConfig.MyOptions.SaveName ?? NowDateTime.ToString("yyyy-MM-dd_HH-mm-ss")}_{task.GetHashCode()}_{OtherUtil.GetValidFileName(streamSpec.GroupId ?? "", "-")}_{streamSpec.Codecs}_{streamSpec.Bandwidth}_{streamSpec.Language}";
        ...
        try
        {
            while ((size = await responseStream.ReadAsync(buffer, CancellationTokenSource.Token)) > 0)
            {
                ...
                RecordingSizeDic[task.GetHashCode()] += size;
        ...
        Logger.InfoMarkUp("File Size: " + GlobalUtil.FormatFileSize(RecordingSizeDic[task.GetHashCode()]));
    ...
    public async Task ReadInfoAsync()
    {
        while (!STOP_FLAG && !READ_IFO)
        {
            ...
            {
                Logger.InfoMarkUp($"Program Id: [cyan]{programId}[/]");
                if (!string.IsNullOrEmpty(serviceName)) Logger.InfoMarkUp($"Service Name: [cyan]{serviceName}[/]");
                if (!string.IsNullOrEmpty(serviceProvider)) Logger.InfoMarkUp($"Service Provider: [cyan]{serviceProvider}[/]");
                READ_IFO = true;
            }
        }
    }
    ...
    public async Task<bool> StartRecordAsync()
    {
        ...
        var dic = SelectedSteams.Select(item =>
        {
            ...
            RecordingDurDic[task.GetHashCode()] = 0;
            RecordingSizeDic[task.GetHashCode()] = 0;
            return (item, task);
        ...
        {
            var task = kp.Value;
            var consumerTask = RecordStreamAsync(kp.Key, task, SpeedContainerDic[task.GetHashCode()]);
