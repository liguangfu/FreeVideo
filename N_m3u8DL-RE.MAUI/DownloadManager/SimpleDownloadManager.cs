...
namespace N_m3u8DL_RE.DownloadManager
{
    internal class SimpleDownloadManager
    {
        ...
        private async Task<bool> DownloadStreamAsync(StreamSpec streamSpec, IProgress<Double> task, SpeedContainer speedContainer)
        {
        ...
        public async Task<bool> StartDownloadAsync()
        {
            ...
            //{
                ...
                if (!DownloaderConfig.MyOptions.ConcurrentDownload)
                {
                    ...
                    {
                        var task = kp.Value;
                        var result = await DownloadStreamAsync(kp.Key, task, SpeedContainerDic[task.GetHashCode()]);
