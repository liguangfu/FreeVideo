...
internal class SimpleLiveRecordManager2
{
    ...
    private async Task<bool> RecordStreamAsync(StreamSpec streamSpec, IProgress<double> task, SpeedContainer speedContainer, BufferBlock<List<MediaSegment>> source)
    {
        ...
        var dirName = $"{task.GetHashCode()}_{OtherUtil.GetValidFileName(streamSpec.GroupId ?? "", "-")}_{streamSpec.Codecs}_{streamSpec.Bandwidth}_{streamSpec.Language}";
        ...
        while (true && await source.OutputAvailableAsync())
        {
            ...
            if (!SamePathDic.ContainsKey(task.GetHashCode()))
            {
                ...
                SamePathDic[task.GetHashCode()] = allSamePath;
            }
            ...
            if (!readInfo || StreamExtractor.ExtractorType == ExtractorType.MSS)
            {
                ...
                //获取文件名
                var filename = GetSegmentName(seg, allHasDatetime, SamePathDic[task.GetHashCode()]);
                var index = seg.Index;
                ...
                && streamSpec.Extension != null && streamSpec.Extension.Contains("ttml"))
            {
                ...
                else
                {
                    foreach (var seg in keys)
                    {
                        ...
                        if (currentVtt.MpegtsTimestamp == 0 && vtt.MpegtsTimestamp == 0)
                        {
                            vtt.MpegtsTimestamp = 90000 * (RecordedDurDic[task.GetHashCode()] + (long)keys.Where(s => s.Index < seg.Index).Sum(s => s.Duration));
                        }
                        currentVtt.AddCuesFromOne(vtt);
                    }
                }
            ...
            RecordedDurDic[task.GetHashCode()] += (int)segmentsDuration;
            ...
            if (DownloaderConfig.MyOptions.LiveRealTimeMerge)
            {
                ...
                if (fileOutputStream == null)
                {
                    ...
                    else 
                    {
                        ...
                        Logger.InfoMarkUp($"{ResString.namedPipeCreated} [cyan]{pipeName}[/]");
                        PipeSteamNamesDic[task.GetHashCode()] = pipeName;
                        if (PipeSteamNamesDic.Count == SelectedSteams.Where(x => x.MediaType != MediaType.SUBTITLES).Count()) 
                        {
                            ...
                            Logger.WarnMarkUp($"{ResString.namedPipeMux} [deepskyblue1]{Path.GetFileName(output)}[/]");
    ...
    private async Task PlayListProduceAsync(Dictionary<StreamSpec, Progress<double>> dic)
    {
        while (!STOP_FLAG)
        {
            if (WAIT_SEC != 0)
            {
                ...
                await Parallel.ForEachAsync(dic, async (dic, _) =>
                {
                    ...
                    //达到上限时 不需要刷新了
                    if (RecordLimitReachedDic[task.GetHashCode()])
                        return;
                    ...
                    //过滤不需要下载的片段
                    FilterMediaSegments(streamSpec, task, allHasDatetime, SamePathDic[task.GetHashCode()]);
                    var newList = streamSpec.Playlist!.MediaParts[0].MediaSegments;
                    if (newList.Count > 0)
                    {
                       // task.MaxValue += newList.Count;
                        //推送给消费者
                        await BlockDic[task.GetHashCode()].SendAsync(newList);
                        //更新最新链接
                        LastFileNameDic[task.GetHashCode()] = GetSegmentName(newList.Last(), allHasDatetime, SamePathDic[task.GetHashCode()]);
                        //尝试更新时间戳
                        var dt = newList.Last().DateTime;
                        DateTimeDic[task.GetHashCode()] = dt != null ? GetUnixTimestamp(dt.Value) : 0L;
                        //累加已获取到的时长
                        RefreshedDurDic[task.GetHashCode()] += (int)newList.Sum(s => s.Duration);
                    }

                    if (!STOP_FLAG && RefreshedDurDic[task.GetHashCode()] >= DownloaderConfig.MyOptions.LiveRecordLimit?.TotalSeconds)
                    {
                        RecordLimitReachedDic[task.GetHashCode()] = true;
                    }
    ...
    private void FilterMediaSegments(StreamSpec streamSpec, IProgress<double> task, bool allHasDatetime, bool allSamePath)
    {
        if (string.IsNullOrEmpty(LastFileNameDic[task.GetHashCode()]) && DateTimeDic[task.GetHashCode()] == 0) return;

        var index = -1;
        var dateTime = DateTimeDic[task.GetHashCode()];
        var lastName = LastFileNameDic[task.GetHashCode()];
        ...
        if (index > -1)
        {
            ...
            if (list.Count > 0)
            {
                ...
                var oldMax = MaxIndexDic[task.GetHashCode()];
                if (newMin < oldMax)
                {
                ...
                MaxIndexDic[task.GetHashCode()] = list.Max(s => s.Index);
            }
    ...
    public async Task<bool> StartRecordAsync()
    {
        ...
        //{
            ...
            {
                var task = new Progress<double>();
                SpeedContainerDic[task.GetHashCode()] = new SpeedContainer(); //速度计算
                //限速设置
                if (DownloaderConfig.MyOptions.MaxSpeed != null)
                {
                    SpeedContainerDic[task.GetHashCode()].SpeedLimit = DownloaderConfig.MyOptions.MaxSpeed.Value;
                }
                LastFileNameDic[task.GetHashCode()] = "";
                RecordLimitReachedDic[task.GetHashCode()] = false;
                DateTimeDic[task.GetHashCode()] = 0L;
                RecordedDurDic[task.GetHashCode()] = 0;
                RefreshedDurDic[task.GetHashCode()] = 0;
                MaxIndexDic[task.GetHashCode()] = item.Playlist?.MediaParts[0].MediaSegments.LastOrDefault()?.Index ?? 0L; //最大Index
                BlockDic[task.GetHashCode()] = new BufferBlock<List<MediaSegment>>();
                return (item, task);
            ...
            {
                var task = kp.Value;
                var consumerTask = RecordStreamAsync(kp.Key, task, SpeedContainerDic[task.GetHashCode()], BlockDic[task.GetHashCode()]);
        ...
        if (success && DownloaderConfig.MyOptions.MuxAfterDone && OutputFiles.Count > 0)
        {
            ...
            OutputFiles.ForEach(f => Logger.WarnMarkUp($"[grey]{Path.GetFileName(f.FilePath)}[/]"));
            var saveDir = DownloaderConfig.MyOptions.SaveDir ?? Environment.CurrentDirectory;
            ...
            Logger.WarnMarkUp($"Muxing to [grey]{outName}{ext}[/]");
            var result = false;
            ...
            {
                Logger.WarnMarkUp($"Rename to [grey]{Path.GetFileName(newPath)}[/]");
