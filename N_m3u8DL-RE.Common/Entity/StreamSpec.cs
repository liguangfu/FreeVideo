...
public class StreamSpec
{
    ...
    public string ToShortString()
    {
        ...
        if (MediaType == Enum.MediaType.AUDIO)
        {
            ...
            returnStr = $"{GroupId} | {(Bandwidth != null ? (Bandwidth / 1000) + " Kbps" : "")} | {Name} | {Codecs} | {Language} | {(Channels != null ? Channels + "CH" : "")} | {Role}";
        }
        else if (MediaType == Enum.MediaType.SUBTITLES)
        {
            prefixStr = $"[deepskyblue3_1]Sub[/] {encStr}";
            returnStr = $"{GroupId} | {Language} | {Name} | {Codecs} | {Role}";
        }
        else
        {
            prefixStr = $"[aqua]Vid[/] {encStr}";
            returnStr = $"{Resolution} | {Bandwidth / 1000} Kbps | {GroupId} | {FrameRate} | {Codecs} | {VideoRange} | {Role}";
        }
    ...
    public string ToShortShortString()
    {
        ...
        {
            prefixStr = $"[deepskyblue3]Aud[/] {encStr}";
            returnStr = $"{(Bandwidth != null ? (Bandwidth / 1000) + " Kbps" : "")} | {Name} | {Language} | {(Channels != null ? Channels + "CH" : "")} | {Role}";
        }
        else if (MediaType == Enum.MediaType.SUBTITLES)
        {
            prefixStr = $"[deepskyblue3_1]Sub[/] {encStr}";
            returnStr = $"{Language} | {Name} | {Codecs} | {Role}";
        }
        else
        {
            prefixStr = $"[aqua]Vid[/] {encStr}";
            returnStr = $"{Resolution} | {Bandwidth / 1000} Kbps | {FrameRate} | {VideoRange} | {Role}";
        }
    ...
    public override string ToString()
    {
        ...
        if (Playlist != null && Playlist.MediaParts.Any(m => m.MediaSegments.Any(s => s.EncryptInfo.Method != EncryptMethod.NONE)))
        {
            ...
            encStr = $"[red]*{string.Join(",", ms)}[/] ";
        }

        if (MediaType == Enum.MediaType.AUDIO)
        {
            prefixStr = $"[deepskyblue3]Aud[/] {encStr}";
            returnStr = $"{GroupId} | {(Bandwidth != null ? (Bandwidth / 1000) + " Kbps" : "")} | {Name} | {Codecs} | {Language} | {(Channels != null ? Channels + "CH" : "")} | {segmentsCountStr} | {Role}";
        }
        else if (MediaType == Enum.MediaType.SUBTITLES)
        {
            prefixStr = $"[deepskyblue3_1]Sub[/] {encStr}";
            returnStr = $"{GroupId} | {Language} | {Name} | {Codecs} | {Characteristics} | {segmentsCountStr} | {Role}";
        }
        else
        {
            prefixStr = $"[aqua]Vid[/] {encStr}";
            returnStr = $"{Resolution} | {Bandwidth / 1000} Kbps | {GroupId} | {FrameRate} | {Codecs} | {VideoRange} | {segmentsCountStr} | {Role}";
        }
